...
| **5.1.0** | **Versione correttiva e di consolidamento** — nessun riallineamento a Tier 0 (resta v6.0.0), nessuna modifica di superficie pubblica. Racchiude tre categorie di modifiche, fin qui presenti nel corpo del documento e/o nel codice ma mai messe a verbale in una voce di changelog dedicata: (1) **tre aggiunte già applicate al corpo del documento nella v5.0.0 ma non versionate**: §8.1.1 (`Utf8CStringBuffer`, contratto stack-first/ArrayPool-fallback), §16.4 (paragrafo Dispose-vs-Interrupt sulla cancellazione a due meccanismi), §20 (voce nel registro rischi sulla fusione `Connection`/`SafeHandle` rimandata, benchmark 0,54×–0,82× vs SQLitePCLRaw); (2) **due difetti reali individuati e corretti nel codice**, emersi scrivendo per la prima volta test che esercitano `SqliteConcurrencyMode.Coordinated` end-to-end (finora privo di qualunque copertura) — **(a)** `DatabaseIdentity.ComputeKey` lanciava `FileNotFoundException` alla primissima apertura di un file non ancora esistente in Coordinated/ReadOnly (il caso comune, dato che Coordinated è la modalità di default): `FileInfo.ResolveLinkTarget` lancia su un percorso assente anziché restituire `null`; corretto con una guardia su `info.Exists` — un file che deve ancora essere creato non può per definizione essere un symlink da risolvere; **(b)** `WalCheckpointCore` passava `zDb=NULL` a `sqlite3_wal_checkpoint_v2`: la documentazione nativa dichiara *pnLog*/*pnCkpt* esplicitamente "undefined" in tal caso, non -1 per convenzione — corretto passando `"main"` esplicito; (3) **correzione di disallineamento documento/codice in §1.6, §15.2, §16.2** (quattro punti distinti): `BackupCoordinator` e `CheckpointCoordinator`, citati come tipi in più punti, non sono mai esistiti nel codice reale (Backup e Checkpoint sono metodi su `Connection`/`SqliteConnection`, non coordinator dedicati); §15.2 citava inoltre `InvalidOperationException` dove il codice reale lancia `SqliteConfigurationException`, e "mode != Passive" per il rifiuto in ReadOnly dove il rifiuto è in realtà incondizionato su tutti i `SqliteCheckpointMode`. **Nuova copertura di test** (§21): `CheckpointTests.cs`, prima suite ad esercitare `SqliteConcurrencyMode.Coordinated`, `BeginTransaction`, `AcquireWriterLeaseAsync` — copre il rifiuto in Native/ReadOnly su tutti i 4 modi, il non-instradamento di `Passive` nel canale del coordinator anche a writer-lease tenuto, e la serializzazione FIFO di `Full`/`Restart`/`Truncate` sia a livello di canale sia end-to-end con una transazione `Immediate` reale su una seconda connessione. **Correzione ulteriore in questa stessa versione**: `AsStructPointer()` sulle 4 `SafeHandle` (`Connection`/`Statement`/`Blob`/`Backup`) rimosso e sostituito, in tutti i ~65 punti di chiamata, dal metodo BCL `DangerousGetHandle()` con cast esplicito — grep-abile in tutto il codice col nome standard .NET, coerente col contratto di §16.4 (eseguito da Francesco Crimi direttamente, verificato su GitHub). |
| **5.2.0** | **Versione correttiva** — nessun riallineamento a Tier 0, nessuna modifica di superficie pubblica, tutta interna al Livello 1/2. Due correzioni distinte, entrambe emerse da una rassegna sistematica dell'intera libreria: (1) **`EngineException.CreateException` controllava solo `IsClosed: false, IsInvalid: false`** — l'unico punto del codebase a farlo, contro le sette occorrenze in `Statement.cs`/`Blob.cs`/`Backup.cs`/`Connection.cs` che controllano correttamente `IsClosed || IsInvalid`. La distinzione conta: `SafeHandle` non azzera mai `handle` dopo `ReleaseHandle()`, quindi `IsInvalid` da solo non rileva un handle già chiuso — un `ConnectionSafeHandle` disposto ha `IsClosed=true` ma `IsInvalid` resta `false`, con rischio di dereferenziare un puntatore nativo già liberato. Nessun percorso di codice attuale dimostrato che lo renda sfruttabile (il contratto di `SqliteThreadingGuard` lo esclude nello scenario più ovvio), ma era l'unico punto disallineato dalla convenzione che il resto del codice si è dato da solo — corretto per coerenza. **Valutata e scartata**, in questa stessa sessione, un'alternativa strutturale: azzerare `handle` dentro `ReleaseHandle()` (così da rendere `IsInvalid` vero da solo dopo il rilascio, senza bisogno del controllo esplicito su `IsClosed`) — corretta nella meccanica (verificato empiricamente), ma scartata perché sovrappone due semantiche che `SafeHandle` tiene deliberatamente distinte per convenzione BCL: `IsInvalid` giudica la validità del valore ricevuto al costruttore, `IsClosed` (mantenuto dalla classe base) segnala l'avvenuta disposizione — collassarle in un solo flag muterebbe il comportamento naturale di `SafeHandle` in un modo che un futuro manutentore, abituato alla convenzione standard, non si aspetterebbe di trovare qui; (2) **`ReleaseHandle()` sulle 4 `SafeHandle`** (`Connection`/`Statement`/`Blob`/`Backup`) **non confronta più l'esito della chiusura nativa con `SQLITE_OK`** — lo esegue e dichiara sempre successo. Un fallimento in chiusura, se reale, deve emergere dal controllo del `ResultCode` sull'operazione che lo ha causato, non essere ricostruito a posteriori nel percorso di rilascio, dove non è più azionabile; dichiarare sempre successo evita inoltre di riattivare la segnalazione interna di `SafeHandle` per un rilascio fallito (rumorosa in alcune configurazioni) per un'informazione ormai inutilizzabile dal chiamante. Stessa logica di principio del punto congelato in questa sessione su `Backup.Step()` (Tier 0 §22): la responsabilità di accorgersi di un errore nativo sta su chi esegue l'operazione, mai su chi la rilascia. **Corretti anche due riferimenti fantasma residui** a `AsStructPointer()` in §16.2 e negli snippet illustrativi di §9 — rinominato ovunque nel codice reale nella 5.1.0 ma sfuggito in tre punti a quella verifica. |
...
| `SingleWriterCoordinator`, `WriterLease`, `CoordinatorRegistry`, `StatementCache`, `PooledConnection` (§9, §11) | **Assente** | Nessuna collisione BCL nota; `PooledConnection` è il nuovo tipo di questa versione (§9.2), stesso trattamento della sua famiglia. |
| `NativeMethods` | **Assente** | Convenzione dell'ecosistema .NET per P/Invoke. |

**Nota su questa versione**: `SqliteResultCursor` è rimosso dall'elenco — non esiste più come tipo (§17). `SqliteConcurrencyMode` guadagna il valore `ReadOnly` e perde `Retry` (§14), senza impatto sulla motivazione del prefisso. **Nota (5.1.0)**: `BackupCoordinator` e `CheckpointCoordinator` sono rimossi dall'elenco — mai esistiti come tipi nel codice reale: Backup e Checkpoint sono metodi (`InitBackup`/`WalCheckpointCore` su `Connection`, proiettati su `SqliteConnection`), non coordinator dedicati (§15).
...
| Async "di comodo" (§16) | `Task.Run` semplice, usato solo da `Backup`/`SqliteConnection.InitBackup` (§15.1) — nessun motore condiviso. |
...
Costruttore che riceve un puntatore già ottenuto, mai un parametro `out`/`ref` marshalled automaticamente. **Corretto (5.2.0)**: `ReleaseHandle()` non confronta più l'esito di `sqlite3_close_v2` con `NativeMethods.SQLITE_OK` — lo esegue e dichiara sempre successo (`return true`). Motivazione: un fallimento in chiusura, se reale, deve emergere dal controllo del `ResultCode` sull'operazione che lo ha causato, non essere ricostruito a posteriori nel percorso di rilascio, dove non è più azionabile; dichiarare sempre successo evita inoltre di riattivare la segnalazione interna di `SafeHandle` per un rilascio fallito (rumorosa in alcune configurazioni) per un'informazione che a quel punto il chiamante non potrebbe comunque usare. Stessa modifica, per lo stesso motivo, su `StatementSafeHandle`, `BlobSafeHandle`, `BackupSafeHandle` (§10, §14, §15).
...
internal sealed unsafe class Connection : IDisposable
{
    ...
    internal sqlite3* NativeHandle => (sqlite3*)_handle.DangerousGetHandle();
    ...
    public void Interrupt() => NativeMethods.sqlite3_interrupt((sqlite3*)_handle.DangerousGetHandle());
...
Invariata la logica per `PASSIVE` (mai instradato) e per `FULL`/`RESTART`/`TRUNCATE` in modalità Coordinated (sempre instradati come turno one-shot attraverso `SingleWriterCoordinator`, I16). **Nuovo in questa versione**: in modalità Native, un checkpoint bloccante è un `PRAGMA wal_checkpoint(...)` diretto, senza garanzia di ordinamento rispetto ad altre scritture — responsabilità del chiamante; in modalità ReadOnly, `SqliteConnection.Checkpoint`/`CheckpointAsync` con qualunque `mode` solleva `SqliteConfigurationException` immediata: nessuna connessione ReadOnly può richiedere un checkpoint, nemmeno `Passive`. **Correzione (5.1.0)**: `WalCheckpointCore` passa ora esplicitamente `zDb="main"` invece di `NULL` — con `NULL`, la documentazione nativa di `sqlite3_wal_checkpoint_v2` dichiara `*pnLog`/`*pnCkpt` "undefined", non popolati in modo affidabile come promesso da `SqliteCheckpointResult`; osservato empiricamente e corretto durante la stesura dei test dedicati (§21). Non toccata dalla correzione: i due contatori restano `-1` — comportamento nativo, non un difetto — se la connessione che esegue il checkpoint non ha essa stessa mai letto o scritto quel database in precedenza (il suo `Pager` non ha ancora agganciato il WAL condiviso).
